...
Crawler.prototype._getUserAgent = function () {
	if (this.userAgent instanceof Array && this.userAgent.length) {
		return this.userAgent[Math.random() * (this.userAgent.length - 1)];
	}
		...
		return urlMatch
			...
			.reduce(function(list,URL) {
				...
				try {
					...
					if (!tmpURL.path().startsWith(".") && !tmpURL.path().startsWith('/')) {
						tmpURL.path("/" + tmpURL.path());
					}
...
Crawler.prototype.fetchQueueItem = function(queueItem) {
	...
	if (crawler.useProxy && crawler.proxies && crawler.proxies.length) {
		var proxy = crawler.proxies[Math.floor(Math.random() * crawler.proxies.length)];
		requestHost = proxy.host;
		requestPort = proxy.port;
	...
	requestOptions = {
		...
		path:	requestPath,
		headers: {
			"User-Agent":	crawler._getUserAgent(),
			"Host":			queueItem.host + (
							queueItem.port !== 80 ?
								":" + queueItem.port :
								""
